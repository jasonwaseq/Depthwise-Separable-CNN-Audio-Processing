  ...
  def _generate_examples(self, archive, file_list):
    ...
    for path, file_obj in archive:
      ...
      if word == BACKGROUND_NOISE:
        ...
        audio = lazy_imports_lib.lazy_imports.pydub.AudioSegment.from_file(
          file_obj, format='wav')
        samples = np.array(audio.get_array_of_samples()).astype(np.float32)
        # Resample to 16kHz mono
        samples = librosa.resample(samples, orig_sr=audio.frame_rate, target_sr=SAMPLE_RATE) if audio.frame_rate != SAMPLE_RATE else samples
        if audio.channels > 1:
          samples = librosa.to_mono(samples.reshape(audio.channels, -1))
        samples = samples / np.max(np.abs(samples))  # Normalize to [-1, 1]
        print(f"[DEBUG] BACKGROUND_NOISE sample_rate: {SAMPLE_RATE}, shape: {samples.shape}")
        for start in range(0, len(samples) - SAMPLE_RATE, SAMPLE_RATE // 2):
          audio_segment = samples[start : start + SAMPLE_RATE]
          cur_id = '{}_{}'.format(example_id, start)
          example = {'audio': audio_segment.astype(np.float32), 'label': label}
          yield cur_id, example
      else:
        try:
          audio = lazy_imports_lib.lazy_imports.pydub.AudioSegment.from_file(file_obj, format='wav')
          samples = np.array(audio.get_array_of_samples()).astype(np.float32)
          # Resample to 16kHz mono
          samples = librosa.resample(samples, orig_sr=audio.frame_rate, target_sr=SAMPLE_RATE) if audio.frame_rate != SAMPLE_RATE else samples
          if audio.channels > 1:
            samples = librosa.to_mono(samples.reshape(audio.channels, -1))
          samples = samples / np.max(np.abs(samples))  # Normalize to [-1, 1]
          print(f"[DEBUG] AUDIO sample_rate: {SAMPLE_RATE}, shape: {samples.shape}")
          example = {
